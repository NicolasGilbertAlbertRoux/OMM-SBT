...
OUT = Path("results/proto_atoms/proto_atom_family_validation")
...
def main():
    ...
    for seed in SEED_LIST:
        ...
        print(
            ...
            f"std={result['final_std']:.3f}"
        )

    df = pd.DataFrame(rows).sort_values("centroid_offset", ascending=True)

    summary_csv = OUT / "family_validation_summary.csv"
    df.to_csv(summary_csv, index=False)

    save_gallery(df, cases, OUT / "family_validation_gallery.png")

    agg = pd.DataFrame([{
        "beta": BETA,
        "center_gain": CENTER_GAIN,
        "node_gain": NODE_GAIN,
        "matter_gain": MATTER_GAIN,
        "flux_gain": FLUX_GAIN,
        "edge_penalty": EDGE_PENALTY,
        "n_seeds": len(df),
        "mean_offset": df["centroid_offset"].mean(),
        "std_offset": df["centroid_offset"].std(),
        "mean_core": df["core_mass_fraction"].mean(),
        "std_core": df["core_mass_fraction"].std(),
        "mean_split": df["split_score"].mean(),
        "std_split": df["split_score"].std(),
        "mean_lobes": df["n_lobes"].mean(),
        "std_lobes": df["n_lobes"].std(),
        "mean_anisotropy": df["anisotropy"].mean(),
        "std_anisotropy": df["anisotropy"].std(),
    }])

    agg_csv = OUT / "family_validation_aggregate.csv"
    agg.to_csv(agg_csv, index=False)

    # meilleur seed pour rendu détaillé
    best_seed = int(df.iloc[0]["seed"])
    best = cases[best_seed]

    fig, axes = plt.subplots(1, 3, figsize=(15, 5))

    axes[0].imshow(best["phi"], cmap="viridis")
    axes[0].set_title(f"Field φ | best seed={best_seed}")

    axes[1].imshow(best["node_field"], cmap="inferno")
    axes[1].set_title("Node field")

    axes[2].imshow(best["occupancy"], cmap="hot")
    ...
    plt.tight_layout()
    best_img = OUT / "family_best_seed_render.png"
    plt.savefig(best_img, dpi=500)
    plt.close(fig)
    ...
    print(f"[OK] wrote {OUT / 'family_validation_gallery.png'}")
    print(f"[OK] wrote {best_img}")
    print("[DONE] proto atom family validation complete")
